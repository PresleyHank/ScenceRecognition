...
def show_results(im, im_scale, line, thresh):
...
# 得到每一个文本框
def get_text_lines(line, thresh):
    inds = np.where(line[:, -1] >= thresh)[0]
    if len(inds) == 0:
        # im = cv2.resize(im, None, None, fx=1.0/im_scale, fy=1.0/im_scale, interpolation=cv2.INTER_LINEAR)
        return
...
def ctpn(sess, net, img):
    ...
    dets = dets[keep, :]
    text_lines = connect_proposal(dets[:, :], dets[:, 4], im.shape[:2])
    tmp = im.copy()
    text_recs = draw_boxes(tmp,text_lines,caption="im_name",wait=True)
    show_results(tmp,im_scale, text_recs, thresh=0.9)
